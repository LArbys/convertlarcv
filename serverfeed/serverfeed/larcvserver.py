...
def __start_workers_v2__(identity,inputfile,address,loadfunc,batchsize,worker_verbosity,queuesize,randomaccess):
    worker = LArCVServerWorker(identity,inputfile,address,loadfunc,batchsize=batchsize,verbosity=worker_verbosity,queuesize=queuesize,randomaccess=randomaccess)
    worker.do_work()
    
class LArCVServer:

    def __init__(self,batchsize,identity,load_func,inputfile,nworkers,server_verbosity=0,worker_verbosity=0,queuesize=1,randomaccess=True):
        ...
        self.pworkers = [ Process(target=__start_workers_v2__,
                                  args=("{}-{}".format(identity,n),inputfile,address,load_func,batchsize,worker_verbosity,queuesize,randomaccess))
